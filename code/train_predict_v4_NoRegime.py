"""
Robust Anomaly Detection v4 - No-Regime Implementation
Optimizations: multi-fold CV, focal loss, cascade architecture,
              temporal smoothing, leakage-safe feature construction,
              feature selection

To run: python code/train_predict_v4_NoRegime.py
Output: submission_v4_NoRegime/pred_simple.csv, submission_v4_NoRegime/pred_complex.csv
"""
import pandas as pd
import numpy as np
import warnings
...
OUTPUT_DIR = 'submission_v4_NoRegime'
...
def fit_lof_pca(source_df, feature_cols):
    """Fit LOF/PCA helpers only on the active training slice to avoid leakage."""
...
# === TRAIN FOLD ===
def train_fold(train_df, train_end, feature_cols):
    ...
    print("  Fitting fold LOF/PCA on train slice...")
...
def main():
    ...
    for fold, train_end in enumerate(CV_SPLITS):
        ...
        models, thresh, auc_pr, f1 = train_fold(train_df, train_end, feature_cols)
    ...
    print("  Fitting final LOF/PCA on final training slice...")
    lof_model, scaler_lof, pca_model, pca_scaler = fit_lof_pca(final_train, feature_cols)
    
    final_fe = create_features(final_train.drop(columns=['y']), feature_cols,
                               lof_model, scaler_lof, pca_model, pca_scaler)
    ...
    pred_complex = apply_temporal_consistency((scores_complex >= base_thresh).astype(int))

"""
Ablation study - GPU accelerated version.
Requires CUDA-enabled XGBoost and LightGBM:
  pip install xgboost[cuda] lightgbm --force-reinstall

Speedup expected: ~3-5x for XGBoost/LightGBM training.
IForest, LOF, PCA remain on CPU (no GPU support).
"""
import pandas as pd
import numpy as np
...
# === GPU CONFIG ===
GPU_ENABLED = True
XGB_GPU_PARAMS = {'device': 'cuda'}       # XGBoost 2.0+: use 'cuda' instead of 'gpu_hist'
...
OUTPUT_DIR = 'ablation_results_gpu'
SPLIT_TRAIN = 130816
SPLIT_VAL = 134545
...
def main():
    print("=" * 70)
    print("ABLATION STUDY (GPU) - v4 Component Analysis")
    print("Split: [:130816] / [130816:134545] / [134545:]")
    if GPU_ENABLED:
        print("GPU: ENABLED (XGBoost cuda + LightGBM gpu)")
    else:
        print("GPU: DISABLED (CPU fallback)")
    print("=" * 70)
    ...
    def predict_ensemble_best(exp_name, models, X, X_sel=None):
        if exp_name == 'E8_full_v4':
            ...
            pred_cascade = cascade_predict(models.get('cascade'), X)
            if pred_cascade is not None:
                base = 0.7 * base + 0.3 * pred_cascade
    ...
    final_models = {}
    # Build whatever the best experiment needs
    if 'xgb_std' in EXPERIMENTS[[e['name'] for e in EXPERIMENTS].index(best['name'])]['models'] \
       or best['name'] == 'E8_full_v4':
        final_models['xgb_std'] = train_xgb_standard(X_full, full_y, X_full, full_y)
    if 'xgb_focal' in EXPERIMENTS[[e['name'] for e in EXPERIMENTS].index(best['name'])]['models'] \
       or best['name'] == 'E8_full_v4':
        final_models['xgb_focal'] = train_xgb_focal(X_full, full_y, X_full, full_y)
    if 'lgb' in EXPERIMENTS[[e['name'] for e in EXPERIMENTS].index(best['name'])]['models'] \
       or best['name'] == 'E8_full_v4':
        final_models['lgb'] = train_lgb(X_full, full_y, X_full, full_y)
    if 'iforest' in EXPERIMENTS[[e['name'] for e in EXPERIMENTS].index(best['name'])]['models'] \
       or best['name'] == 'E8_full_v4':
        final_models['iforest'] = train_iforest(X_full, full_y)
    if 'cascade' in EXPERIMENTS[[e['name'] for e in EXPERIMENTS].index(best['name'])]['models'] \
       or best['name'] == 'E8_full_v4':
        final_models['cascade'] = train_cascade(X_full, full_y, X_full, full_y)
    if 'sel' in EXPERIMENTS[[e['name'] for e in EXPERIMENTS].index(best['name'])]['models'] \
       or best['name'] == 'E8_full_v4':
        ...
        X_test_simple_sel = X_test_complex_sel = None
    ...
    scores_simple = predict_ensemble_best(best['name'], final_models, X_test_simple, X_test_simple_sel)
    scores_complex = predict_ensemble_best(best['name'], final_models, X_test_complex, X_test_complex_sel)
